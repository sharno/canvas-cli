...
- Sections/enrollments: sections at `/courses/:course_id/sections` and `/sections/:id`; enrollments at `/sections/:section_id/enrollments` with `enrollment[type]=StudentEnrollment|TaEnrollment|TeacherEnrollment` plus `limit_privileges_to_course_section` when needed.
- Notification prefs: list channels via `/users/self/communication_channels`, then fetch `/users/self/communication_channels/:id/notification_preferences`; update via `/users/self/communication_channels/:id/notification_preferences/:notification` with frequency payload.
- Ask planning: parse prompt via `canvas_core::parse_ask_prompt`, then plan with `RuleBasedAskPlanner` and emit params as `key`/`value` objects.
